...
class AuthService:
    ...
    async def login(self, data: LoginRequest) -> LoginResponse:
        ...
        # 1. Fetch user profile (Fixed method name)
        user = await self.repository.find_by_email(email_clean)
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid email or password", # Best practice: don't reveal if email exists
            )

        # 2. Enforce Lockout Checking BEFORE running CPU-bound password checks
        if user.get("locked_until"):
            # Ensure timezone uniformity during extraction
        ...
        # 3. Check password accuracy safely in worker threads
        password_valid = await asyncio.to_thread(self._verify_password, data.password, user["password_hash"])
        
        if not password_valid:
            # 4. Trigger the atomic execution counter (Do not wrap this in asyncio.to_thread!)
